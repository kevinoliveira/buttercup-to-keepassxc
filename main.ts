...
const groupNameIndex: Record<string, string> = groupLineObjects.reduce(
	(acc, cur) => ({
		[cur.group_id]: cur.group_name,
		...acc,
	}),
	{},
);
...
const groupParrentIndex: Record<string, string | null> =
	groupLineObjects.reduce(
		(acc, cur) => ({
			[cur.group_id]: cur.group_parent || null,
			...acc,
		}),
		{},
	);
...
const isGroupInTrash = (groupId: string) => {
	if (
		groupNameIndex[groupId] === trashBinGroupName &&
		groupParrentIndex[groupId] === null
	)
		return true;
	if (groupParrentIndex[groupId])
		return isGroupInTrash(groupParrentIndex[groupId]);
	return false;
};
...
const entriesToIgnore = entryObjectLines.filter((lo) =>
	isGroupInTrash(lo.group_id),
);
const entriesToExport = entryObjectLines.filter(
	(lo) => !isGroupInTrash(lo.group_id),
);

const entriesFormated: Array<EntryParsed> = entriesToExport
